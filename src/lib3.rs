...
impl RepeatUntil<[Player; 2]> {
    ...
    fn repeat_by(mut self, fallback: fn() -> String) -> State {
        let call = Box::clone(&self.state.stack[self.state.level]);
        ...
        self.state.stack[self.state.level] = Box::new(closure);
        State { ..self.state }
    }
}
impl<T> Until<T> {
...
type InpAction = Box<(dyn Fn(Game) -> String + Send + Sync + 'static)>;
struct State {
    game: Game,
    stack: Vec<Action>,
    level: usize,
...
impl State {
    fn start_by(get_inp: fn() -> String) -> Until<[Player; 2]> {
        ...
        Until::<[Player; 2]> {
            recall_stack: Box::new(move |game| get_inp()),
            state,
            p: PhantomData,
        }
    }
    ...
    fn after_that(self, f: fn(Game) -> Game) -> Self {
        let b = [self.stack, vec![Box::new(move |game| f(game))]].concat();
        Self {
            game: self.game,
            stack: b,
            level: self.level + 1,
